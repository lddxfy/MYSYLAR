...
public:
    ...
    static typename ConfigVar<T>::ptr Lookup(const std::string& name,const T& default_value,const std::string& description = ""){
        ...
        if(it != GetDatas().end()){
            ...
            if(tmp){
                LAMB_LOG_INFO(LAMB_LOG_ROOT())<<"Lookup name="<< name << " exists";
                return tmp;
            }else {
                LAMB_LOG_INFO(LAMB_LOG_ROOT())<<"Lookup name=" << name << " exists but type not "
        ...
        if(name.find_first_not_of("abcdefghikjlmnopqrstuvwxyz._0123456789") != std::string::npos){
            LAMB_LOG_INFO(LAMB_LOG_ROOT())<<"Lookup name invalid " << name;
