...
int IOManager::addEvent(int fd, Event event, std::function<void()> cb) {
    ...
    if (LAMB_UNLIKELY(fd_ctx->events & event)) {
        LAMB_LOG_ERROR(g_logger) << "addEvent assert fd=" << fd
                                  << " event=" << (EPOLL_EVENTS)event
                                  << " fd_ctx.event=" << (EPOLL_EVENTS)fd_ctx->events;
        LAMB_ASSERT(!(fd_ctx->events & event));
    }
    ...
    if (rt) {
        LAMB_LOG_ERROR(g_logger) << "epoll_ctl(" << m_epfd << ", "
    ...
    LAMB_ASSERT(!event_ctx.scheduler && !event_ctx.fiber && !event_ctx.cb);
    ...
    if (cb) {
        ...
        LAMB_ASSERT2(event_ctx.fiber->getState() == Fiber::RUNNING, "state=" << event_ctx.fiber->getState());
    }
    return 0;
}
...
void IOManager::tickle() {
    LAMB_LOG_DEBUG(g_logger) << "tickle";
    if(!hasIdleThreads()) {
        return;
    }
...
void IOManager::idle() {
    ...
    while (true) {
        ...
        if( LAMB_UNLIKELY(stopping(next_timeout))) {
            LAMB_LOG_DEBUG(g_logger) << "name=" << getName() << "idle stopping exit";
            break;
        }
        ...
        do{
            ...
            LAMB_LOG_DEBUG(g_logger) <<"rt = " <<rt << " ,errno = "<< errno;
            if(rt < 0 && errno == EINTR) {
                continue;
            } else {
                break;
