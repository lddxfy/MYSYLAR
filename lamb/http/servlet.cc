...
int32_t FunctionServlet::handle(lamb::http::HttpRequest::ptr request
                   , lamb::http::HttpResponse::ptr response
                   , lamb::http::HttpSession::ptr session){
    return m_cb(request,response,session);
}

ServletDispatch::ServletDispatch()
    :Servlet("ServletDispatch"){
    m_default.reset(new NotFoundServlet("lamb/1.0"));
}


int32_t ServletDispatch::handle(lamb::http::HttpRequest::ptr request
                   , lamb::http::HttpResponse::ptr response
                   , lamb::http::HttpSession::ptr session){
...
void ServletDispatch::addGlobServlet(const std::string& uri, FunctionServlet::callback cb){
    return ServletDispatch::addGlobServlet(uri,std::make_shared<FunctionServlet>(cb));
}
...
int32_t NotFoundServlet::handle(lamb::http::HttpRequest::ptr request
                   , lamb::http::HttpResponse::ptr response
                   , lamb::http::HttpSession::ptr session){

    response->setStatus(lamb::http::HttpStatus::NOT_FOUND);
    response->setHeader("Server", "lamb/1.0.0");
