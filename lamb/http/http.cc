    ...
    do {                                                                                                   \
        ...
        if (0) {                                                                                           \
            std::cout << "<key>:" << str.substr(last, key - last)                                          \
                      << " <decoded>:" << lamb::StringUtil::UrlDecode(str.substr(last, key - last))       \
                      << " <value>:" << str.substr(key + 1, pos - key - 1)                                 \
                      << " <decoded>:" << lamb::StringUtil::UrlDecode(str.substr(key + 1, pos - key - 1)) \
                      << std::endl;                                                                        \
        }                                                                                                  \
                                                                                                           \
        m.insert(std::make_pair(lamb::StringUtil::UrlDecode(trim(str.substr(last, key - last))),          \
                                lamb::StringUtil::UrlDecode(str.substr(key + 1, pos - key - 1))));        \
        if (pos == std::string::npos) {                                                                    \
            break;                                                                                         \
        }                                                                                                  \
        ++pos;                                                                                             \
...
void HttpRequest::initCookies(){
    ...
    PARSE_PARAM(cookie,m_cookies,";",lamb::StringUtil::Trim);
}
...
void HttpResponse::setCookie(const std::string& key, const std::string& val,
    ...
    ss << key <<"="<<val;
    if(expired > 0) {
        ss << ";expired="<< lamb::Time2Str(expired, "%a, %d %b %Y %H:%M:%S") << " GMT";
    }
    if(!domain.empty()){
        ss << ";domain=" <<domain;
    }
