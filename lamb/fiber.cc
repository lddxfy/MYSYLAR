...
Fiber::Fiber(){
    ...
    LAMB_LOG_DEBUG(g_logger) << "Fiber::Fiber() main id =" << m_id;
}
...
Fiber::Fiber(std::function<void()> cb,size_t stacksize,bool run_in_scheduler) : m_id(s_fiber_id++),m_cb(cb),m_runInScheduler(run_in_scheduler){
    ...
    LAMB_LOG_DEBUG(g_logger) << "Fiber::Fiber() id = " << m_id;
}
...
Fiber::~Fiber(){
    ...
    if(m_stack){
        ...
        LAMB_ASSERT(m_state == TERM);
        StackAllocator::Dealloc(m_stack,m_stacksize);
        LAMB_LOG_DEBUG(g_logger) << "dealloc stack, id = " << m_id;
    }else {
        // 没有栈，说明是线程的主协程
        LAMB_ASSERT(!m_cb); // 主协程没有cb
        LAMB_ASSERT(m_state == RUNNING); // 主协程一定是执行状态
    }
...
Fiber::ptr Fiber::GetThis(){
    ...
    LAMB_ASSERT(t_fiber == main_fiber.get());
    t_thread_fiber = main_fiber;
    return t_fiber->shared_from_this();
}

void Fiber::resume(){
    LAMB_ASSERT(m_state != TERM && m_state != RUNNING)
    SetThis(this);
    m_state = RUNNING;
    // 如果协程参与调度器调度，那么应该和调度器的主协程进⾏swap，⽽不是线程主协程
    if(m_runInScheduler){
        if(swapcontext(&(Scheduler::GetMainFiber()->m_ctx),&m_ctx)){
            LAMB_ASSERT2(false,"swapcontext");
        }
    }else{
...
void Fiber::yield(){
    LAMB_ASSERT(m_state == RUNNING || m_state == TERM);
