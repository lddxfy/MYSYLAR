...
lamb::ConfigVar<std::set<LogDefine>>::ptr g_log_defines = 
    lamb::Config::Lookup("logs",std::set<LogDefine>(),"logs config");

struct LogIniter {
    LogIniter() {
        // 添加变化回调函数
        g_log_defines->addListener([](const std::set<LogDefine>& old_value,const std::set<LogDefine>& new_value){
            LAMB_LOG_INFO(LAMB_LOG_ROOT()) << "on_logger_conf_changed";
            for(auto& i : new_value){
                auto it = old_value.find(i);
                lamb::Logger::ptr logger;
                // new有 old没有
                if(it == old_value.end()){
                    logger = LAMB_LOG_NAME(i.name);
                }
                else{
                    if(!(i == *it)){
                        // 修改的logger
            ...
            for(auto& i : old_value){
                ...
                if(it==new_value.end()){
                    //删除logger
                    auto logger = LAMB_LOG_NAME(i.name);
