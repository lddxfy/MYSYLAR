...
static lamb::Mutex s_mutex;

/// 是否强制加载配置文件，非强制加载的情况下，如果记录的文件修改时间未变化，则跳过该文件的加载
void Config::LoadFromConfDir(const std::string &path,bool force){
    std::string absoulte_path = lamb::EnvMgr::GetInstance()->getAbsolutePath(path);
    ...
    for(auto &i : files){
        ...
        try {
            ...
            LoadFromYaml(root);
            //LAMB_LOG_INFO(g_logger) << "LoadConfFile file = " << i << " ok";
            //LAMB_LOG_DEBUG(g_logger) << "LoadConfFile file = " << i << " ok";
        }catch(...){
            LAMB_LOG_ERROR(g_logger) << "LoadConfFile file =" << i <<" failed";
        }
    }
