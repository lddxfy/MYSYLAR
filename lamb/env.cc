...
namespace lamb
{
        ...
        for (int i = 1; i < argc; ++i)
        {
            if (argv[i][0] == '-')
            {
                if (strlen(argv[i]) > 1)
                {
                    ...
                    LAMB_LOG_ERROR(g_logger) << "invalid arg idx =" << i << " val=" << argv[i];
                    return false;
                }
            }else {
                if(now_key) {
                    ...
                    LAMB_LOG_ERROR(g_logger) << "invalid are idx=" << i <<"val=" << argv[i];
                    return false;
                }
            }
        }
    ...
    std::string Env::getAbsoluteWorkPath(const std::string& path) const{
        ...
        static lamb::ConfigVar<std::string>::ptr g_server_work_path =
            lamb::Config::Lookup<std::string>("server.work_path");
