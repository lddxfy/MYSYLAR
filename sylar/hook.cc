...
sylar::Logger::ptr g_logger = MYSYLAR_LOG_NAME("system");
namespace sylar {
...
struct _HookIniter {
    _HookIniter() {
        ...
        g_tcp_connect_timeout->addListener([](const int& old_value, const int& new_value){
                MYSYLAR_LOG_INFO(g_logger) << "tcp connect timeout changed from "
...
retry:
    ...
    if(n == -1 && errno == EAGAIN) {
        ...
        if(SYLAR_UNLIKELY(rt)) {
            MYSYLAR_LOG_ERROR(g_logger) << hook_fun_name << " addEvent("
                << fd << ", " << event << ")";
            if(timer) {
                timer->cancel();
            }
...
int connect_with_timeout(int fd, const struct sockaddr* addr, socklen_t addrlen, uint64_t timeout_ms) {
    ...
    if(rt == 0) {
        ...
        MYSYLAR_LOG_ERROR(g_logger) << "connect addEvent(" << fd << ", WRITE) error";
    }
