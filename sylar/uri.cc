...
Uri::ptr Uri::Create(const std::string& urlstr){
    ...
    if(parser.field_set & (1 << UF_SCHEMA)){
        uri->setScheme(std::string(urlstr.c_str() + parser.field_data[UF_SCHEMA].off,parser.field_data[UF_SCHEMA].len));
    }

    if(parser.field_set & (1 << UF_USERINFO)){
        uri->setUserinfo(std::string(urlstr.c_str() + parser.field_data[UF_USERINFO].off,parser.field_data[UF_USERINFO].len));
    }
...
int32_t Uri::getPort() const{
    ...
    if(m_scheme == "http" || m_scheme == "ws"){
        return 80;
    }else if(m_scheme == "https" || m_scheme == "ws"){
        return 443;
    }
    return m_port;
}
