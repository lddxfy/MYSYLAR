...
#define LOG_LEVEL(logger,level)\
    if(logger->getLevel() <= level) \
        sylar::LogEventWrap(logger,sylar::LogEvent::ptr(new sylar::LogEvent(\
            ...
            syscall(SYS_gettid),sylar::GetFiberId(),time(0),sylar::GetThreadName()\
        ))).getSS()
...
#define MYSYLAR_LOG_INFO(logger) LOG_LEVEL(logger,sylar::LogLevel::INFO)
#define MYSYLAR_LOG_DEBUG(logger) LOG_LEVEL(logger,sylar::LogLevel::DEBUG)
#define MYSYLAR_LOG_ERROR(logger) LOG_LEVEL(logger,sylar::LogLevel::ERROR)
#define MYSYLAR_LOG_FATAL(logger) LOG_LEVEL(logger,sylar::LogLevel::FATAL)
#define MYSYLAR_LOG_WARN(logger) LOG_LEVEL(logger,sylar::LogLevel::WARN)


#define MYSYLAR_LOG_ROOT() sylar::LoggerMgr::GetInstance()->getRoot()
#define MYSYLAR_LOG_NAME(name) sylar::LoggerMgr::GetInstance()->getLogger(name)
