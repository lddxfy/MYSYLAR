...
namespace sylar {
    ...
    */
    ...
    bool Socket::getOption(int level,int option,void *result,socklen_t *len){
        ...
        if(rt){
            MYSYLAR_LOG_DEBUG(g_logger) << "getOption sock=" << m_sock
    ...
    bool Socket::setOption(int level, int option, const void *result, socklen_t len){
        if(setsockopt(m_sock,level,option,result,(socklen_t)len)){
            MYSYLAR_LOG_DEBUG(g_logger) << "setOption sock=" << m_sock
                                  << " level=" << level << " option=" << option
                                  << " errno=" << errno << " errstr=" << strerror(errno);
            return false;
        }
    ...
    Socket::ptr Socket::accept(){
        ...
        if(newsock == -1){
            MYSYLAR_LOG_ERROR(g_logger) << "accept(" << m_sock << ") errno="
    ...
    bool Socket::bind(const Address::ptr addr){
        ...
        if(SYLAR_UNLIKELY(addr->getFamily() != m_family)){
            MYSYLAR_LOG_ERROR(g_logger) << "bind sock.family("
        ...
        if(::bind(m_sock,addr->getAddr(),addr->getAddrLen())){
            MYSYLAR_LOG_ERROR(g_logger) << "bind error errno =" << errno
    ...
    bool Socket::connect(const Address::ptr addr, uint64_t timeout_ms){
        ...
        if(timeout_ms == (uint64_t)-1){
            if(::connect(m_sock,addr->getAddr(),addr->getAddrLen())){
                MYSYLAR_LOG_ERROR(g_logger) << "sock=" << m_sock << " connect(" << addr->toString()
    ...
    bool Socket::reconnect(uint64_t timeout_ms){
        if(!m_remoteAddress){
            MYSYLAR_LOG_ERROR(g_logger) << "reconnect m_remoteAddress is null";
            return false;
        }
    ...
    bool Socket::listen(int backlog ){
        if(!isValid()){
            MYSYLAR_LOG_ERROR(g_logger) << "listen error sock=-1";
            return false;
        }
        if(::listen(m_sock,backlog)==-1){
            MYSYLAR_LOG_ERROR(g_logger) << "listen error errno=" << errno
                                  << " errstr=" << strerror(errno);
            return false;
        }
        return true;
    ...
    Address::ptr Socket::getRemoteAddress(){
        ...
        //MYSYLAR_LOG_INFO(g_logger) << m_family;
        Address::ptr result;
        switch (m_family)
        {
        case AF_INET:
        ...
        if(getpeername(m_sock,result->getAddr(),&addrlen)){
            MYSYLAR_LOG_ERROR(g_logger) << "getpeername error sock=" << m_sock
                                  << " errno=" << errno << " errstr=" << strerror(errno);
    ...
    Address::ptr Socket::getLocalAddress(){
        ...
        if(getsockname(m_sock,result->getAddr(),&addrlen)){
            MYSYLAR_LOG_ERROR(g_logger) << "getsockname error sock=" << m_sock
                                  << " errno=" << errno << " errstr=" << strerror(errno);
            return Address::ptr(new UnknownAddress(m_family));
        }
    ...
    void Socket::newSock(){
        ...
        if(SYLAR_LIKELY(m_sock != -1)){
            ...
            MYSYLAR_LOG_ERROR(g_logger) << "socket(" << m_family
