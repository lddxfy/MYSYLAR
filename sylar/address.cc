...
namespace sylar
{
    ...
    bool Address::Lookup(std::vector<Address::ptr> &result, const std::string &host, int family, int type , int protocol)
    {
        ...
        if (error)
        {
            MYSYLAR_LOG_DEBUG(g_logger) << "Address::Lookup getaddress(" << host << ", "
        ...
        while (next)
        {
            ...
            MYSYLAR_LOG_DEBUG(g_logger) << "family:" << next->ai_family << ", sock type:" << next->ai_socktype;
            next = next->ai_next;
        }
                                        ...
                                        int family)
    {
        ...
        {
            MYSYLAR_LOG_DEBUG(g_logger) << "Address::GetInterfaceAddresses getifaddrs "
                                           " err="
        ...
        {
            MYSYLAR_LOG_ERROR(g_logger) << "Address::GetInterfaceAddresses exception";
            freeifaddrs(results);
            return false;
        }
        freeifaddrs(results);
    ...
    IPAddress::ptr IPAddress::Create(const char *address, uint16_t port)
    {
        ...
        if (error)
        {
            MYSYLAR_LOG_DEBUG(g_logger) << "IPAddress::Create(" << address
                                        << ", " << port << ") error=" << error
    ...
    IPv4Address::ptr IPv4Address::Create(const char *address, uint16_t port)
    {
        ...
        if (result <= 0)
        {
            MYSYLAR_LOG_DEBUG(g_logger) << "IPv4Address::Create(" << address << ", "
    ...
    IPAddress::ptr IPv4Address::broadcastAddress(uint32_t prefix_len)
    {
        ...
        // byteswapOnLittleEndian(CreateMask<uint32_t>(prefix_len));主机部分地址
    ...
    IPv6Address::ptr IPv6Address::Create(const char *address, uint16_t port)
    {
        ...
        if (result <= 0)
        {
            MYSYLAR_LOG_DEBUG(g_logger) << "IPv6Address::Create(" << address << ", "
                                        << port << ") rt=" << result << " errno=" << errno
                                        << " errstr=" << strerror(errno);
            return nullptr;
        }
