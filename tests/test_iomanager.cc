...
void do_io_write() {
    ...
    if(so_err) {
        MYSYLAR_LOG_INFO(g_logger) << "connect fail";
        return;
    } 
    MYSYLAR_LOG_INFO(g_logger) << "connect success";
}
...
void do_io_read() {
    ...
    if(readlen > 0) {
        buf[readlen] = '\0';
        MYSYLAR_LOG_INFO(g_logger) << "read " << readlen << " bytes, read: " << buf;
    } else if(readlen == 0) {
        MYSYLAR_LOG_INFO(g_logger) << "peer closed";
        close(sockfd);
        return;
    } else {
        MYSYLAR_LOG_ERROR(g_logger) << "err, errno=" << errno << ", errstr=" << strerror(errno);
        close(sockfd);
        return;
    }
...
void watch_io_read() {
    MYSYLAR_LOG_INFO(g_logger) << "watch_io_read";
...
void test_io() {
    ...
    if(rt != 0) {
        if(errno == EINPROGRESS) {
            MYSYLAR_LOG_INFO(g_logger) << "EINPROGRESS";
            ...
            MYSYLAR_LOG_ERROR(g_logger) << "connect error, errno:" << errno << ", errstr:" << strerror(errno);
        }
    } else {
        MYSYLAR_LOG_ERROR(g_logger) << "else, errno:" << errno << ", errstr:" << strerror(errno);
    }
}
