 ...
 */
void test_fiber1() {
    MYSYLAR_LOG_INFO(g_logger) << "test_fiber1 begin";
    ...
    MYSYLAR_LOG_INFO(g_logger) << "before test_fiber1 yield";
    sylar::Fiber::GetThis()->yield();
    MYSYLAR_LOG_INFO(g_logger) << "after test_fiber1 yield";

    MYSYLAR_LOG_INFO(g_logger) << "test_fiber1 end";
}
 ...
 */
void test_fiber2() {
    MYSYLAR_LOG_INFO(g_logger) << "test_fiber2 begin";
    ...
    MYSYLAR_LOG_INFO(g_logger) << "test_fiber2 end";
}

void test_fiber3() {
    MYSYLAR_LOG_INFO(g_logger) << "test_fiber3 begin";
    MYSYLAR_LOG_INFO(g_logger) << "test_fiber3 end";
}

void test_fiber5() {
    static int count = 0;

    MYSYLAR_LOG_INFO(g_logger) << "test_fiber5 begin, i = " << count;
    MYSYLAR_LOG_INFO(g_logger) << "test_fiber5 end i = " << count;
 ...
 */
void test_fiber4() {
    MYSYLAR_LOG_INFO(g_logger) << "test_fiber4 begin";
    
    for (int i = 0; i < 3; i++) {
        sylar::Scheduler::GetThis()->schedule(test_fiber5, sylar::GetThreadId());
    }

    MYSYLAR_LOG_INFO(g_logger) << "test_fiber4 end";
}
